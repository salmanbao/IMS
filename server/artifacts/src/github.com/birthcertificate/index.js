...
let Chaincode = class {
    ...
    async addBirthCertificate(stub) {
    ...
    async DeleteBirthCertificate(stub, args) {
    ...
    async getBirthCertificate(stub, args) {
        console.info('============= Get Birth Certificate===========');
        if (args.length != 1) {
            throw new Error('Incorrect number of arguments. Expecting 1');
        }
        try {
            ...
            return Buffer.from(JSON.stringify(certificate));
        } catch (error) {
